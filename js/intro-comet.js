(function () {
  ...
  function runIntro() {
    ...
    // Plain Canvas2D + requestAnimationFrame only (no WebGL, no other
    // unsupported APIs), so this runs the same on iOS Safari as anywhere.
    ...
    // visualViewport reflects the true visible area on iOS Safari, where
    // innerHeight can be thrown off by the collapsing address/tab bar.
  ...
  // Primary trigger: run immediately on page load, on every device.
  runIntro();

  // Backup trigger: some mobile browsers can delay/deprioritize JS work
  // that isn't tied to a user gesture. If runIntro() hasn't already
  // fired by the first touch, this catches it — the hasRun guard means
  // it's a no-op (and never double-plays) if the load trigger worked.
  window.addEventListener('touchstart', runIntro, { once: true, passive: true });
})();
